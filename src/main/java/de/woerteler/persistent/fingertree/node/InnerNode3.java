 ...
 */
public class InnerNode3<N, E> extends InnerNode<N, E> {
  /** First sub-node. */
  public final Node<N, E> child0;
  /** Second sub-node. */
  public final Node<N, E> child1;
  /** Third sub-node. */
  public final Node<N, E> child2;
   ...
   */
  public InnerNode3(final Node<N, E> a, final Node<N, E> b, final Node<N, E> c) {
    this.child0 = a;
    this.l = a.size();
    this.child1 = b;
    this.m = l + b.size();
  ...
  public InnerNode<N, E> reverse() {
    return new InnerNode3<N, E>(child2, child1, child0);
  }
  ...
  public Node<N, E>[] remove(final long pos) {
    ...
    @SuppressWarnings("unchecked")
    final Node<N, E>[] out = new Node[] { a, b };
    return out;
  }
  ...
  public NodeLike<Node<N, E>, E> slice(final long start, final long len) {
    ...
    if(start < l) {
      ...
      if(end <= m) {
        // no right node
        if(end <= l) return new OneSubnode<N, E>(slice0);
        ...
        return merged[1] == null ? new OneSubnode<>(merged[0]) :
      ...
      return p2 == 1 ? new OneSubnode<>(ns[0])
    ...
    if(start < m) {
      ...
      if(end <= m) return new OneSubnode<>(slice1);
    ...
    return new OneSubnode<N, E>(slice2);
  }
