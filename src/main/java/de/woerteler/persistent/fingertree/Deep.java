...
import de.woerteler.persistent.fingertree.node.Node.*;
 ...
 */
public final class Deep<N, E> extends FingerTree<N, E> {
  /** Left digit. */
  final Node<N, E>[] left;
  ...
  public Deep<N, E> cons(final Node<N, E> fst) {
    if(left.length < 4) {
      ...
      newLeft[0] = fst;
      return new Deep<N, E>(newLeft, leftSize + fst.size(), middle, right, size + fst.size());
    }
   ...
   */
  public FingerTree<N, E> remove(final long pos) {
    if(pos < leftSize) return removeLeft(pos);

    final long midSize = middle.size();
    if(pos - leftSize < midSize) return removeMiddle(pos - leftSize);

    return removeRight(pos - leftSize - midSize);
  }
  ...
  /**
   * Remove an element from the middle tree.
   * @param pos position inside the middle tree
   * @return resulting tree
   */
  private FingerTree<N, E> removeMiddle(final long pos) {
    if(middle instanceof Deep) {
      // middle cannot underflow
      final Deep<Node<N, E>, E> deep = (Deep<Node<N, E>, E>) middle;
      return new Deep<>(left, leftSize, deep.remove(pos), right, size - 1);
    }

    // delete from single middle node
    final Single<Node<N, E>, E> single = (Single<Node<N, E>, E>) middle;
    final Node<N, E>[] rem = ((InnerNode<N, E>) single.elem).remove(pos);
    if(rem.length > 1) {
      // no underflow
      final Node<Node<N, E>, E> newNode =
          rem.length == 2 ? new InnerNode2<>(rem[0], rem[1])
                          : new InnerNode3<>(rem[0], rem[1], rem[2]);
      return new Deep<>(left, leftSize, new Single<>(newNode), right, size - 1);
    }
    ...
    if(left.length < right.length) {
      ...
      newLeft[left.length] = rem[0];
      return Deep.get(newLeft, leftSize + rem[0].size(), right, size - 1);
    }
    ...
    final Node<Node<N, E>, E> newMid = new InnerNode3<>(left[3], rem[0], right[0]);
    final Node<N, E>[] newRight = slice(right, 1, right.length);
    return Deep.get(newLeft, new Single<>(newMid), newRight, size - 1);
  }
  ...
  public TreeSlice<N, E> slice(final long from, final long len) {
    ...
    if(inLeft == len) return TreeSlice.fromNodes(buffer, inBuffer, len);
    ...
    if(inMiddle == 0) {
      ...
      TreeSlice<Node<N, E>, E> slice = middle.slice(midOff, inMiddle);
      if(slice.isPartial()) {
        final NodeLike<N, E> sub = ((OneSubnode<N, E>) slice.getPartial()).sub;
    ...
    if(mid.isEmpty()) {
      ...
      return TreeSlice.fromNodes(buffer, inBuffer, len);
    }
    ...
    return new TreeSlice<>(Deep.get(newLeft, mid3, newRight, len));
  }
  ...
  /**
   * Iterator over a finger tree with at least two elements.
   *
   * @author Leo Woerteler
   * @param <E> element type
   */
  private static class DeepIterator<E> implements Iterator<E> {
    /** Stack of deep nodes. */
    ...
    /** Sub-iterator over elements in a node. */
    private final NodeIterator<E> sub;
     ...
     */
    @SuppressWarnings("unchecked")
    DeepIterator(final Deep<?, E> root) {
      trees = new Deep[8];
      poss = new byte[8];
      sp = 0;
      trees[0] = root;
      sub = new NodeIterator<>(root.left[0]);
      poss[0] = 1;
    }
    ...
    public E next() {
      ...
      if(!sub.hasNext()) {
        ...
        final Deep<?, E> curr = trees[sp];
        if(poss[sp] >= 0) {
          // we are in the left digit
          if(poss[sp] < curr.left.length) {
            // go to next node in digit
            sub.init(curr.left[poss[sp]]);
            poss[sp]++;
          } else {
            // left digit drained
            ...
            if(mid instanceof Empty) {
              ...
              sub.init(curr.right[0]);
              poss[sp] = -2;
            } else if(mid instanceof Single) {
              // iterate through the one middle node
              sub.init(((Single<?, E>) mid).elem);
              poss[sp] = -1;
            } else {
              final Deep<?, E> deep = (Deep<?, E>) mid;
              if(++sp == trees.length) {
                trees = Arrays.copyOf(trees, 2 * trees.length);
                poss = Arrays.copyOf(poss, 2 * poss.length);
              }
              trees[sp] = deep;
              sub.init(deep.left[0]);
              poss[sp] = 1;
            }
          }
        } else {
          // we are in the right digit
          final int pos = -(poss[sp] + 1);
          if(pos < curr.right.length) {
            // go to next node in digit
            sub.init(curr.right[pos]);
            poss[sp]--;
          } else {
            // backtrack one level
            sp--;
            if(sp >= 0) {
              sub.init(trees[sp].right[0]);
              poss[sp] = -2;
            }
          }
        }
