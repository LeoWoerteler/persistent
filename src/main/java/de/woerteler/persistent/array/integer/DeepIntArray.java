...
final class DeepIntArray extends IntArray {
  ...
  public IntArray concat(final Array<Integer> seq) {
    ...
    return new DeepIntArray(left, middle.concat(midNodes, n, other.middle), other.right);
  }
  ...
  public IntArray remove(final long pos) {
    ...
    TreeSlice<Integer, Integer> slice = middle.remove(pos - left.length);

    if(slice.isTree()) {
      // middle tree did not underflow
      return new DeepIntArray(left, slice.getTree(), right);
    }

    // tree height might change
    final int[] mid = ((PartialIntLeaf) slice.getPartial()).elems;
    ...
    if(l > r) {
      ...
      final int[] newMid = slice(left, l - move, l + m);
      System.arraycopy(mid, 0, newMid, move, m);
      return new DeepIntArray(newLeft, FingerTree.singleton(new IntLeaf(newMid)), right);
    }

    if(r > MIN_DIGIT) {
      // steal from right digit
      final int move = (r - MIN_DIGIT + 1) / 2;
      final int[] newMid = slice(mid, 0, m + move);
      System.arraycopy(right, 0, newMid, m, move);
      final int[] newRight = slice(right, move, r);
      return new DeepIntArray(left, FingerTree.singleton(new IntLeaf(newMid)), newRight);
    }
